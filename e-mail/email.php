...
{
    if(sendMail($_POST['email'],'contato@servidorinformatica.com.br', $_POST['nome'].'<BR/><BR/>'.'MENSAGEM'.'<BR/>'.$_POST['mensagem'], 'Formulário de contato - Servidor Informática'))
    {
        echo $_POST['nome']." "."sua mensagem foi enviada com sucesso!";
		echo '<br/>';
		echo "Você vai receber uma cópia desta mensagem em seu email!";
		echo '<br/>';
		echo "Em breve entraremos em contato!";
		sendMail($_POST['email'],$email, $_POST['nome'].'<BR/><BR/>'.'MENSAGEM'.'<BR/>'.$_POST['mensagem'], 'ENC: Formulário de contato - Servidor Informática');
    }
    else
    {
...
<body>
	<h2>Formulário de contato - Servidor Informática</h2>
